      ...
      <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {services.map(({ slug, icon: Icon, navTitle, short }) => (
          <Link
            key={slug}
            href={`/servicos/${slug}`}
            className="group flex flex-col rounded-3xl border border-line bg-white p-6 transition-all hover:-translate-y-1 hover:border-primary/30 hover:shadow-[var(--shadow-card)]"
          >
